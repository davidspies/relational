...
$$F_r \cdot \overline{active_{r,\text{cand}}} + F_r \cdot active_{r,\text{check}} + \sum_{b_i \in B^+} w_i \cdot \overline{b_{i,\text{check}}} + \sum_{c_j \in B^-} u_j \cdot c_{j,\text{check}} \geq F_r$$
...
2. Find **external support**: for each atom $x \in U$ and each rule $r$ where $x \in heads(r)$:
   - Let $overlap = \sum \{w_i : b_i \in body^+(r) \cap U\}$ (sum of weights of positive body atoms in $U$)
   - Let $s = t_r + overlap$ (required support level)
   - If $s > W_r$: rule cannot provide external support (skip)
   - Otherwise: add $active_{r,x,s,\text{cand}}$ (for non-choice) or $active_{r,s,\text{cand}}$ (for choice)
3. Add to the candidate solver:
$$\sum_{x \in U} \overline{x_{\text{cand}}} + \sum_{(r,x,s) \in \text{external}} active_{r,x,s,\text{cand}} \geq 1$$
