...
Initially, the bottom solver is solving the ASP program _nearly_ naively encoded as SAT with no supportedness constraints. It contains the variables x_bottom for each atom x in the ASP program, and active_r for each rule r in the ASP program. So if you have a rule r which says:

h :- b1, b2, b3, not b4.

We'll encode that as:

active_r v not b1_bottom v not b2_bottom v not b3_bottom v b4_bottom

not active_r v b1_bottom
not active_r v b2_bottom
not active_r v b3_bottom
not active_r v not b4_bottom

h_bottom v not active_r

We export that as the external relation (both the bottom variables and the rule activeness variables).

The _top_ solver has two more variables for each ASP atom, x_top and x_diminished. These aren't external, they're free in the top solver, but come with the following constraints:
...
not active_r v h_top v not b1_top v not b2_top v not b3_top (we can omit b4, because it's not positive)
