...
impl Solver {
    ...
    pub fn new(
        ...
        // Learned constraints
        create_persistent_input!(
            db,
            learned_terms_inp,
            learned_terms,
            (ConstraintId, Lit, Weight)
        );
        create_persistent_input!(
            db,
            learned_bounds_inp,
        ...
        create_input!(db, analysis_inp, analysis_conflict, Conflict);
        ...
        let (analysis_constraint_ids, new_clause) =
            conflict_analysis(db, &all_terms, &causes, analysis_conflict);
        ...
        assign!(
            propagate_lit_level_cause = propagate_with_level.map(|((cid, lit), level)| (
                lit,
                level,
                Cause::FromConstraint(cid)
            ))
        );
        ...
        Solver {
            ...
            outputs: Outputs {
                ...
                analysis_constraint_ids: analysis_constraint_ids.boxed().output(),
...
fn conflict_analysis(
    ...
    Relation<impl Op<(Lit, Level)> + 'static>,
) {
    assign_partition!(
        (analysis_start_constraint_id, analysis_start_var) =
            conflict.map(|conflict| match conflict {
    ...
    assign!(analysis_start_var_lits = analysis_start_var.flat_map(|v| [Lit::pos(v), Lit::neg(v)]));
    ...
    assign!(
        analysis_new_lits = all_terms
            .get()
            .map(|(cid, lit, _weight)| (cid, !lit))
            .join_values(analysis_constraint_ids.get()) // (neg_lit, explain_commit_id)
            .join(
                causes
                    .get()
